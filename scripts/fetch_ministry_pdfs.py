...
class ComprehensivePDFFetcher:
    ...
    async def fetch_pdfs_for_ministry(self, ministry):
        ...
        while has_more_pages and page <= max_pages:
            try:
                ...
                questions = await self.sansad_client.fetch_questions(ministry=ministry, page=page)
                
                if not questions:
                    logger.info(f"No more questions found for {ministry} after page {page-1}")
                    has_more_pages = False
                    break
                
                logger.info(f"Found {len(questions)} questions for {ministry} on page {page}")
                self.stats["total_pdfs_found"] += len(questions)
                
                # Download PDFs for ALL questions on this page
                for question in tqdm(questions, desc=f"{ministry} page {page}", leave=False):
    ...
    async def fetch_all_ministries(self):
        ...
        print(f"Ministries processed: {self.stats['ministries_processed']}/{len(Config.MINISTRIES)}")
