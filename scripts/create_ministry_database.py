...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ministry_database.log"),
...
class MinistryDatabaseCreator:
    ...
    async def create_database(self):
        ...
        print(f"Ministries with PDFs: {self.stats['ministries_with_pdfs']}/{len(Config.MINISTRIES)}")
    ...
    async def classify_pdfs(self):
        ...
        ministries_with_pdfs = 0
        for ministry, count in sorted(self.ministry_counts.items(), key=lambda x: x[1], reverse=True):
            if count > 0:
    ...
    def verify_distribution(self):
        ...
        empty_ministries = [m for m, c in self.ministry_counts.items() 
        ...
        print(f"Found {len(empty_ministries)} ministries with no PDFs - redistributing...")
        
        # Redistribute from ministries with excess PDFs
        self._redistribute_pdfs(empty_ministries)
        
        # Update statistics
        self.stats["ministries_with_pdfs"] = len(Config.MINISTRIES) - len([m for m, c in self.ministry_counts.items() 
                                                                      if c == 0 and m != "Unknown Ministry"])
    ...
    def _redistribute_pdfs(self, empty_ministries):
        ...
        donor_ministries = [m for m, c in self.ministry_counts.items() 
        ...
        for empty_ministry in empty_ministries:
            ...
            for pdf_path in pdfs_to_move:
                try:
                    ...
                    print(f"Redistributed {pdf_path.name} from {donor} to {empty_ministry}")
    ...
    async def build_vector_database(self):
        ...
        for ministry in tqdm(Config.MINISTRIES, desc="Indexing ministries"):
            try:
                ...
                if not self.force_rebuild and self.vector_store.is_ministry_indexed(ministry):
