            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        logger.info(
                            f"Successfully fetched questions for {ministry} (page {page})"
                        )
                ...
                if retry_count == max_retries:
                    logger.error(
                        f"Failed to fetch questions after {max_retries} attempts: {e}"
                    )
                    return []
        ...
        try:
            ...
            while retry_count < Config.MAX_RETRIES:
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            formatted_url, headers=self.headers, timeout=Config.TIMEOUT
                        ) as response:
                    ...
                    retry_count += 1
                    logger.warning(
                        f"Response error downloading PDF (attempt {retry_count}): {e}"
                    )
                    if retry_count == Config.MAX_RETRIES:
                        logger.error(
                            f"Failed to download PDF after {Config.MAX_RETRIES} attempts: {e}"
                        )
                        return None
                    ...
                    retry_count += 1
                    logger.warning(
                        f"Client error downloading PDF (attempt {retry_count}): {e}"
                    )
                    if retry_count == Config.MAX_RETRIES:
                        logger.error(
                            f"Failed to download PDF after {Config.MAX_RETRIES} attempts: {e}"
